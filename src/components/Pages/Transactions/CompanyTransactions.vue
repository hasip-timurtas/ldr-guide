        ...
        <v-row class="pa-4">
          ...
          <v-col cols="12">
            ...
            <v-row>
              ...
              ></v-text-field>
              <v-spacer></v-spacer>
              <v-btn color="info" small @click="handleDownloadExcel">Excel İndir</v-btn>
            </v-row>
          </v-col>
        </v-row>
      </v-card-title>
...
export default {
  ...
  computed: {
    newTransactions() {
      return this.companyTransactions.filter(
        e =>
          e.status == "completed" &&
  ...
  methods: {
    handleRedirect(transaction) {
      this.$router.push("/view-transaction/" + transaction._id + "/company");
    },
    handleDownloadExcel() {
      // format data for downloading
      const data = this.companyTransactions.map(e => {
        return {
          ID: e._id,
  ...
  created() {
    if (this.userLevel !== "company") {
      // user company değilse bu sayfayı görmesin gerek yok.
      this.$router.push({ name: "PendingTransactions" });
    }

    this.setDates();
    this.loadCompanyTransactions(this.user.email);
  }
};
</script>
